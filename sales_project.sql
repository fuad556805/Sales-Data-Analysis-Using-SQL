...
with Ordered as (
    select ProductKey, sum(OrderQuantity) as TotalOrdered
    from fact_sales
    group by ProductKey
),
Returned as (
...
select 
    ...
    r.TotalReturned,
    o.TotalOrdered,
    round((r.TotalReturned / o.TotalOrdered) * 100, 2) as Return_Percentage
from product p
join Ordered o on p.ProductKey = o.ProductKey
join Returned r on p.ProductKey = r.ProductKey
